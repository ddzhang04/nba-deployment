          ...
          <div className="guess-history-aside">
            {!isPostGameView ? (
              <>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <button type="button" onClick={() => setGuessHistorySort('score')} style={{ padding: '8px 12px', borderRadius: '999px', border: '1px solid rgba(59,130,246,0.35)', backgroundColor: guessHistorySort === 'score' ? 'rgba(59,130,246,0.18)' : 'rgba(15,23,42,0.35)', color: guessHistorySort === 'score' ? '#93c5fd' : '#94a3b8', cursor: 'pointer', fontWeight: 700, font: 'inherit', fontSize: '12px' }} title="Show highest scores first">Score</button>
                    <button type="button" onClick={() => setGuessHistorySort('chronological')} style={{ padding: '8px 12px', borderRadius: '999px', border: '1px solid rgba(52,211,153,0.35)', backgroundColor: guessHistorySort === 'chronological' ? 'rgba(52,211,153,0.16)' : 'rgba(15,23,42,0.35)', color: guessHistorySort === 'chronological' ? '#6ee7b7' : '#94a3b8', cursor: 'pointer', fontWeight: 700, font: 'inherit', fontSize: '12px' }} title="Show guesses in the order you made them">Chronological</button>
                  </div>
                </div>
                {guessHistory.length === 0 ? (
                  loading ? (
                    <div style={{ padding: '12px 0' }}><div style={{ marginBottom: '12px', backgroundColor: '#0f172a', borderRadius: 12, border: '1px solid #334155', padding: 16 }}><div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}><div className="nm-skeleton" style={{ width: 40, height: 40, borderRadius: 8 }} /><div className="nm-skeleton" style={{ flex: 1, height: 16, borderRadius: 8 }} /></div><div className="nm-skeleton" style={{ width: '100%', height: 24, borderRadius: 12 }} /></div></div>
                  ) : (
                    <div style={{ textAlign: 'center', color: '#64748b', padding: '20px 12px', fontSize: '0.92rem' }}><div style={{ fontSize: '1.9rem', marginBottom: '8px', opacity: 0.85 }}>🔍</div><p style={{ margin: 0 }}>No guesses yet.</p></div>
                  )
                ) : (
                  <div className="nm-guess-history-scroll">
                    {(guessHistorySort === 'chronological' ? guessHistory : guessHistory.slice().sort((a, b) => b.score - a.score)).map((item, index) => (
                      <div key={index} ref={item.name === pulseGuessName ? pulseGuessCardRef : null} className={['nm-guess-card', item.name === pulseGuessName ? 'nm-pulse' : ''].filter(Boolean).join(' ')} style={{ backgroundColor: 'rgba(15, 23, 42, 0.65)', borderRadius: '10px', padding: '13px', marginBottom: '9px', border: '1px solid rgba(51, 65, 85, 0.75)', textAlign: 'left' }}>
                        <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '10px' }}>{renderPlayerAvatar(item.name, { size: 36, radius: 8 })}<h4 style={{ margin: 0, color: '#e2e8f0', fontSize: '1rem', fontWeight: 800, flex: 1, minWidth: 0 }}>{item.name}</h4></div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', flex: 1, minHeight: 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1, minHeight: 0, borderBottom: '1px solid rgba(71, 85, 105, 0.55)', paddingBottom: '8px' }}>
                  ...
                  <div className="nm-guess-history-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                    {guessHistory.length === 0 ? (
                      <div style={{ textAlign: 'center', color: '#64748b', padding: '16px 10px', fontSize: '0.92rem' }}>No guesses yet.</div>
                    ) : (
                      (guessHistorySort === 'chronological' ? guessHistory : guessHistory.slice().sort((a, b) => b.score - a.score)).map((item, index) => (
                        <div key={index} ref={item.name === pulseGuessName ? pulseGuessCardRef : null} className={['nm-guess-card', item.name === pulseGuessName ? 'nm-pulse' : ''].filter(Boolean).join(' ')} style={{ backgroundColor: 'rgba(15, 23, 42, 0.65)', borderRadius: '10px', padding: '13px', marginBottom: '9px', border: '1px solid rgba(51, 65, 85, 0.75)', textAlign: 'left' }}>
                          <div style={{ marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '10px' }}>{renderPlayerAvatar(item.name, { size: 36, radius: 8 })}<h4 style={{ margin: 0, color: '#e2e8f0', fontSize: '1rem', fontWeight: 800, flex: 1, minWidth: 0 }}>{item.name}</h4></div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', flex: 1, minHeight: 0 }}>
                  <h3 style={{ fontSize: '1rem', margin: 0, color: '#e2e8f0', fontWeight: 800 }}>📈 Top 5 Most Similar</h3>
                  <div className="nm-guess-history-scroll" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
                    {top5Players.length > 0 ? (
                      top5Players.map(([name, score], index) => (
                        <div key={name} style={{ marginBottom: '11px', backgroundColor: 'rgba(15, 23, 42, 0.55)', border: '1px solid rgba(71,85,105,0.6)', borderRadius: '10px', padding: '10px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '7px' }}>
                            <span style={{ backgroundColor: '#3b82f6', color: 'white', width: '24px', height: '24px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 'bold', flex: '0 0 auto' }}>{index + 1}</span>
                      ...
                      <div style={{ color: '#cbd5e1', fontSize: '0.9rem' }}>{restoringTop5 ? 'Loading this daily Top 5...' : 'Generating closest guesses...'}</div>
                    ) : (
